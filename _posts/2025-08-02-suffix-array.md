...
Problem is, if you dump a tens-of-terabytes-large dataset onto HuggingFace, even if it's "open" to everyone, it's still hard to know what *is* in the dataset.
...
The original idea was, larger LLMs are more persistent at completing well-known phrases with the common ending word (e.g., completing the proverb *"What everybody says must be"* with *"true"*), even when instructed otherwise (e.g., *"Write a sentence about challenging common beliefs"*).
...
The main difference is scale -- in CP we build SAs for up to $10^6$ items, but now we need to deal with $10^{12}$ items.
...
*Illustration of the suffix array on a toy example.*
...
Many work, including my infini-gram, adapt their code for SA indexing (albeit a few bugs and inefficiencies which I fixed while learning to read Rust), and I really appreciate their awesome release.
...
Not only did we achieve the biggest in terms of reference data (5 trillion tokens, beating [previous record](https://aclanthology.org/D07-1090.pdf) set by Jeff Dean's team at Google), but we also support arbitrarily large context length $n$.
...
*The Speech and Language Processing textbook mentions infini-gram.*
...
Folks around me also shared feedback that being able to search for co-occurrence of multiple terms would be super useful, so I invented an fast algorithm to search for [CNF queries](https://en.wikipedia.org/wiki/Conjunctive_normal_form) (which I even forgot to describe in the paper).
...
With the help of Zihao, I moved to [Pybind11](https://github.com/pybind/pybind11) to interface C++ and Python and this has been really good.
...
I devised a two-tier pre-fetching strategy: at each binary search step, prefetch SA entries $s$ steps ahead, and prefetch the suffix $r$ steps ahead (with $r < s$).
After some tuning on the production server, I found $s = 3$ and $r = 1$ to give the lowest latency.
With the SA index stored on AWS EBS gp3 SSDs (16000 IOPS, 1000 MB/s), the average latency of the `find()` operation was about 20 milliseconds.
...
One thing I noticed was that my instance would OOM and went down every few days, and the devil turned out to be the **page table**.
...
By default, the mmap uses a lazy strategy to populate this page table on demand, but as more disk blocks get accessed, the page table grows and there's no apparent way to evict it from RAM.
...
Overall, I'm really glad that I flashed out all those systems, and I want to express my deep gratitute to my advisors for kindly offering the cloud credits to let me keep the service running.
...
I was messing around with this idea in mid 2024, and eventually I joined efforts with Ai2 to build this tool, **OLMoTrace**.

### The techincal part

Apparently, we can't expect a multi-hundred-token LLM output to exist contiguously in the training data (unless it's regurgitating some well-known stuff), so we should look for substrings (i.e. span of tokens) of the LLM output that do exist.
Since the training data is so huge, we can actually find a lot of long spans (10 tokens or more) with a match.
...
A quick back-of-envelope math: Each `find()` is 80 disk reads, and we need to multiply by 12 because the data is so huge that we need to shard the SA 12 ways; each LLM output is about 450 tokens; this gives us $80 * 12 * (450^2 / 2) = 97M$ disk IOs.
...
We adopted this heuristic when using infini-gram to compute the [Creativity Index](https://arxiv.org/pdf/2410.04265) of text, which we define based on n-gram novelty.
The algorithm, which we dubbed as "DJ search", reduces the queries from $L^2 / 2$ to $2L$ sequential ones, and considering each query being 20ms this gives us $(2 * 450) * 20 \text{ms} = 18$ seconds per LLM output.
...
The key idea is that we only need to find the "maximal matching spans" in the LLM output that exist in the training data, which can be reduced to making 1 `find()` query per suffix of the LLM output.
...
By default, most machines give us 1 million threads per process, which is pretty generous, but in our system we actually need to watch out for this.
...
But we didn't release until April 2025, and this was mainly because we've been polishing OLMoTrace as a product.
...
If a user sees this training document on top, they may get the wrong message from our tool.
To address this, we applied a reranker to surface the most relevant matched training documents in the UI.
We found a BM25 reranker to be roughly as good as neural embedding models in terms of perceived relevance (via human eval), so we went with BM25 to avoid needing GPU machines in the production system.

We also decluttered the UI so as to not overwhelm users.
There can be many "maximal matching spans" to show, some of which overlapping, which would mess up with the span highlights.
...
We also ran an internal **red teaming** to understand and mitigate the legal risks, including copyrighted books, lyrics, and toxic text.
...
Working as a team means I need to forego my bad research-y coding habits, and instead write unit tests, linting my code, creating and reviewing PRs, etc.
...
I spent some effort to align these with their role on the project and motivate our team members.
...
An interesting lesson I learned was how to report to leadership.
...
Sometimes it means having to asking them to look at cherry-picked examples before we iron out use cases more broadly.
